...
function _CancelProgressAnimations() {
    if (gAsyncAbort !== null) {
        gAsyncAbort.abort();
        gAsyncAbort = null;
    }
    ...
    // todo(ericl): any cleanup necessary?
}
function* WaitForPanelClass(panel, panelClass, abortSignal) {
    while (!panel.BHasClass(panelClass) && !abortSignal.aborted) {
        yield Async.NextFrame();
    }
}
function* AnimateXPGain(panel, xpAmount, sReason) {
    ...
    overheadDisplay.SetDialogVariableLocString("reason", sReason);
    overheadDisplay.SetDialogVariableInt("xp_amount", xpAmount);
    ...
    while (true) {
        ...
        const xpGain = Math.floor(progress * xpAmount);
        badge.progressheroxp = xpStart + xpGain;
        if (curTime >= startTime + duration)
            break;
        yield Async.NextFrame();
    }
    badge.progressheroxp = xpStart + xpAmount;
    overheadDisplay.DeleteAsync(0.0);
}
function* DoProgressAnimationOld(panel, data, abortSignal) {
    ...
    // Default all the stats
    for (let i = 0; i < localPlayer.stats.length; ++i) {
        let stat = localPlayer.stats[i];
        let statPanel = panel.FindChildInLayoutFile(stat.stat_panel_id);
        statPanel.SetDialogVariableInt("stat_value", stat.stat_value);
        statPanel.RemoveClass("ShowStat");
        statPanel.RemoveClass("HasAward");
    }
    // Wait for the hero model to load
    yield* WaitForPanelClass(heroScenePanel, "SceneLoaded", abortSignal);
    heroScenePanel.ApplyStyles(true);
    //heroScenePanel.PlayPanelSound( "UI.PostGame.ProgressStart" );
    heroScenePanel.PlayPanelSound(localPlayer.match_outcome_sound);
    panel.AddClass("ShowHeroModel");
    yield Async.Delay(0.5);
    panel.AddClass("ShowScoreboardEntry");
    yield Async.Delay(1.0);
    // Animate win/loss xp
    panel.PlayPanelSound("UI.PostGame.Award");
    yield* AnimateXPGain(panel, localPlayer.match_hero_xp, localPlayer.win ? "#Citadel_HeroXP_Reason_Win" : "#Citadel_HeroXP_Reason_Loss");
    // Animate the stats appearing
    for (let i = 0; i < localPlayer.stats.length; ++i) {
        let stat = localPlayer.stats[i];
        let statPanel = panel.FindChildInLayoutFile(stat.stat_panel_id);
        statPanel.AddClass("ShowStat");
        statPanel.PlayPanelSound("UI.PostGame.StatAppear");
        yield Async.Delay(0.2);
        if (stat.award_xp > 0) {
            statPanel.AddClass("HasAward");
            statPanel.PlayPanelSound("UI.PostGame.Award");
            yield* AnimateXPGain(panel, stat.award_xp, stat.award_title ?? "");
        }
    }
...
// Called from C++
function StartProgressAnimationOld(data) {
    $.Msg("Running progress animation:");
    $.Msg(JSON.stringify(data));
    _CancelProgressAnimations();
    const panel = $.GetContextPanel();
    gAsyncAbort = new Async.AbortController();
    Async.RunSequence(function* (abortSignal) { yield* DoProgressAnimationOld(panel, data, abortSignal); }, gAsyncAbort.signal);
}
